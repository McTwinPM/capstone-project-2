...
class DrinkRecipeList(Resource):
    ...
    def post(self):
        try:
            ...
            return jsonify({"message": "Drink recipe created successfully", "drink_recipe": drink_recipe_schema.dump(drink_recipe)}), 201
        except ValidationError as err:
            return jsonify(err.messages), 400
...
class DrinkRecipeDetail(Resource):
    def get(self, recipe_id):
        ...
        if not drink_recipe:
            return jsonify({"error": "Drink recipe not found"}), 404
        
        drink_recipe_schema = DrinkRecipeSchema()
        return jsonify(drink_recipe_schema.dump(drink_recipe)), 200
    ...
    def patch(self, recipe_id):
        ...
        try:
            ...
            db.session.commit()
            return jsonify({"message": "Drink recipe updated successfully", "drink_recipe": drink_recipe_schema.dump(drink_recipe)}), 200
        except ValidationError as err:
            return jsonify(err.messages), 400
    def delete(self, recipe_id):
        ...
        db.session.commit()
        return jsonify({"message": "Drink recipe deleted successfully"}), 200
